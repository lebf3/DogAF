...
# sampleTable$sex <- factor(c("F","F","M","M","F","M","M","F","F","M","M","M","M","F","M","F","M","M"))
...
```{r}
lfg_CTLvsAVB <- as.data.frame(lfcShrink(dds, contrast=c("condition","CTL","AVB"), type = "normal"))
colnames(lfg_CTLvsAVB) <- paste0('lfg_CTLvsAVB', colnames(lfg_CTLvsAVB))

lfg_CTLvsATP <- as.data.frame(lfcShrink(dds, contrast=c("condition","CTL","ATP"), type = "normal"))
colnames(lfg_CTLvsATP) <- paste0('lfg_CTLvsATP', colnames(lfg_CTLvsATP))

lfg_AVBvsATP <- as.data.frame(lfcShrink(dds, contrast=c("condition","AVB","ATP"), type = "normal"))
...
```{r}
# from STAR alignment
geneinfo <- data.table::fread("data/final/geneInfo.tab")
