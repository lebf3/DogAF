...
lbl <- ifelse(mRNA$CTLvsAVBpadj < 10^-10, mRNA$Gene, NA)
lbl[which(mRNA$GENEID == "ENSCAFG00000049959")] <- "MEG3"
...
lbl <- ifelse(mRNA$CTLvsATPpadj < 10^-10, mRNA$Gene, NA)
lbl[which(mRNA$GENEID == "ENSCAFG00000049959")] <- "ENSCAFG00000049959(MEG3)"
...
#######################
lbl <- ifelse(mRNA$AVBvsATPpadj < 10^-11, mRNA$Gene, NA)
lbl[which(mRNA$GENEID == "ENSCAFG00000049959")] <- "MEG3"
...
for (i in 1:3) {
  l[[paste0("Up_",substr(colnames(mRNA)[padj_cols[i]],1,8))]] <- ifelse(mRNA[,padj_cols[i]] < 0.01 & mRNA[,LFC_cols[i]] > 0, TRUE, FALSE)
  l[[paste0("Down_",substr(colnames(mRNA)[padj_cols[i]],1,8))]] <- ifelse(mRNA[,padj_cols[i]] < 0.01 & mRNA[,LFC_cols[i]] < 0, TRUE, FALSE)
}
...
colnames(df) <- gsub("CTLvsATP","AFvsCTL",colnames(df))
colnames(df) <- gsub("CTLvsAVB","AF+AVBvsCTL",colnames(df))
colnames(df) <- gsub("AVBvsATP","AFvsAF+AVB",colnames(df))

# compare significant genes sets
upset_p <- upset(df, colnames(df), width_ratio=0.2,sort_sets='ascending', stripes=c(NA , NA ),
                 set_sizes = upset_set_size(
                   geom=geom_bar(fill="darkred")),
...
png("manuscript/Figure4.png", units="in", bg = "transparent", width=12, height=9, res=600)
