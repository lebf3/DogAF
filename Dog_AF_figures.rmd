...
## 1.c-PCA after correction for fibroblast fraction
...
p <- ggdraw() +
  ...
  draw_plot(PCA_cor, x = 0.2, y = 0, width = 0.6, height = 0.5) +
  draw_plot_label(label = c("A", "B", "C"), size = 15,
                  x = c(0, 0.66, 0.2), y = c(1, 1, 0.5))
...
t.14q32 <- data.frame(CTLvsAVB.fdr = mRNA[match(unique(targets_names),mRNA$Gene),"CTLvsAVBpadj"], gene = unique(targets_names))
...
# Side barplot showing fdr for top 5 pathways
...
p3 <- ggplot(data = t.14q32, aes(x = reorder(gene, CTLvsAVB.fdr), y = -log10(CTLvsAVB.fdr))) +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        panel.border=element_blank())+
  annotate("text", x = -4, y = 5, label = "~-Log[10]~~italic(FDR )~ AF+AVBvsCTL", angle = 90, size = 3, parse = TRUE) +
  coord_cartesian(xlim = c(1, 82), ylim = c(0, 10), clip = "off")
